...
```ts
import createMDX from '@vezham/docs-mdx/config';
...
Now you can generate types using `@vezham/docs-mdx` command, it's recommended to set it as a `postinstall` script.
...
```ts
import { map } from '@/.map';
import { createMDXSource } from '@vezham/docs-mdx/runtime/next';
...
```ts
import { defineDocs, defineCollections, frontmatterSchema } from '@vezham/docs-mdx/config';
import { z } from 'zod';
