...
```ts
import createMDX from '@vx-oss/docs-mdx/config';
...
Now you can generate types using `@vx-oss/docs-mdx` command, it's recommended to set it as a `postinstall` script.

```json title="package.json"
{
  "scripts": {
    "postinstall": "@vx-oss/docs-mdx"
  }
}
```
...
```ts
import { map } from '@/.map';
import { createMDXSource } from '@vx-oss/docs-mdx/runtime/next';
...
```ts
import { defineDocs, defineCollections, frontmatterSchema } from '@vx-oss/docs-mdx/config';
import { z } from 'zod';
