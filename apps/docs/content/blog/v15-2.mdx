...
Fumadocs 15.2 introduces a layer over `@vezham/docs-core` to extend support to other React frameworks, including React Router and Tanstack Start. Other frameworks can also be supported using the `FrameworkProvider` from `@vezham/docs-core/framework/base`.
...
This is a minor release, it shouldn't break any previous usages unless you're relying on the lower level APIs from `@vezham/docs-core` without `@vezham/docs-react`.
