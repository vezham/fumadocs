...
Fumadocs 15.2 introduces a layer over `@vx-oss/docs-core` to extend support to other React frameworks, including React Router and Tanstack Start. Other frameworks can also be supported using the `FrameworkProvider` from `@vx-oss/docs-core/framework/base`.
...
This is a minor release, it shouldn't break any previous usages unless you're relying on the lower level APIs from `@vx-oss/docs-core` without `@vx-oss/docs-react`.
