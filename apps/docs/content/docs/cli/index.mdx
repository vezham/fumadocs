...
```package-install
npx @vx-oss/docs-cli add
```

You can pass component names directly.

```package-install
npx @vx-oss/docs-cli add banner files
```
...
```package-install
npx @vx-oss/docs-cli tree ./my-dir ./output.tsx
```

You can output MDX files too:

```package-install
npx @vx-oss/docs-cli tree ./my-dir ./output.mdx
```

See help for further details:

```package-install
npx @vx-oss/docs-cli tree -h
```

#### Example Output

```tsx title="output.tsx"
import { File, Folder, Files } from '@vx-oss/docs-react/components/files';
