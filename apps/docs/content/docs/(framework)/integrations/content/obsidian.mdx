...
`@vx-oss/docs-obsidian` exposes a runtime content source. It reads and compiles the vault directly, so it does not generate intermediate MDX files or need a Fumadocs MDX remark plugin.
...
```package-install
@vx-oss/docs-obsidian shiki
```

### Generate Styles

The components from `@vx-oss/docs-obsidian/ui` are styled with Tailwind CSS. Add the following line so their classes are generated:
...
/* [!code ++] */
@import '@vx-oss/docs-obsidian/css/preset.css';
```
...
import { DocsBody, DocsPage } from '@vx-oss/docs-react/layouts/docs/page';
...
{
  "scripts": {
    "dev": "@vx-oss/docs-obsidian dev -- next dev"
  }
}
```
...
import { localContentPlugin } from '@vx-oss/docs-obsidian/dev/vite';
...
```ts title="lib/source.ts"
import { watchWithVite } from '@vx-oss/docs-obsidian/dev/vite';
...
```ts title="lib/source.ts"
import { frontmatterSchema, obsidian } from '@vx-oss/docs-obsidian';
import { metaSchema } from '@vx-oss/docs-core/source/schema';
import { z } from 'zod';
