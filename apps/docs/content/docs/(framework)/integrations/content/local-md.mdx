...
`@vx-oss/docs-local-md` is a content source for local Markdown/MDX files, it is bundleless (works fully at runtime) by design.
...
import { localContentPlugin } from '@vx-oss/docs-local-md/dev/vite';
...
```ts title="lib/source.ts"
import { watchWithVite } from '@vx-oss/docs-local-md/dev/vite';
...
```ts title="lib/source.ts"
import { watchWithDevServer } from '@vx-oss/docs-local-md/dev/ws';
...
When compiling Markdown files (`*.md`), `@vx-oss/docs-local-md` uses a virtual JavaScript engine to avoid `eval()` at runtime.
...
import { defineDocs, defineConfig } from '@vx-oss/docs-mdx/config';
...
With `@vx-oss/docs-local-md`, you can replace that setup with:
