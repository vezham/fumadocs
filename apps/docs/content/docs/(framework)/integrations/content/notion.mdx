...
`@vx-oss/docs-notion` turns a Notion database into a Fumadocs content source. It provides a dynamic [content source](/docs/headless/source-api) that pulls pages over the official Notion API, and a server renderer that renders Notion blocks with Fumadocs UI.
...
```package-install
@vx-oss/docs-notion @notionhq/client
```

Import the renderer's Tailwind CSS source after the Fumadocs UI styles:

```css title="app/global.css"
@import '@vx-oss/docs-react/css/neutral.css';
@import '@vx-oss/docs-react/css/preset.css';
/* [!code ++] */
@import '@vx-oss/docs-notion/css/preset.css';
```
...
import { createNotion } from '@vx-oss/docs-notion';
...
} from '@vx-oss/docs-notion';
import { DocsPage, DocsBody, DocsDescription, DocsTitle } from '@vx-oss/docs-react/layouts/docs/page';
...
import { createNotionFileHandler } from '@vx-oss/docs-notion';
import { notion } from '@/lib/notion';
...
```tsx
import { NotionRenderer } from '@vx-oss/docs-notion';
...
Code that only pulls Notion pages, such as a search indexer or `llms.txt` route, can import from `@vx-oss/docs-notion/source` to skip the React renderer entry:

```ts
import { createNotion } from '@vx-oss/docs-notion/source';
```
