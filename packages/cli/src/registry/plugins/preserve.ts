...
/**
 * keep references to `@vx-oss/docs-react/layouts/*` components as original, unless the user is installing them directly.
 */
export function pluginPreserveLayouts(): InstallerPlugin {
  ...
  return {
    ...
    transformImport(specifier, { stack }) {
      ...
      return `@vx-oss/docs-react/${layoutComps[specifier]}`;
    },
  };
}
